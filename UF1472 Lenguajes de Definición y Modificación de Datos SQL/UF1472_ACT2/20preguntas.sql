...
FROM clientes c
JOIN pedidos p ON c.id = p."id de cliente"
WHERE p."fecha de pedido" >= DATEADD(month, -1, GETDATE());
...
SELECT MONTH(p."fecha de pedido") AS Mes, SUM(p."gastos de envío") AS TotalGastosEnvio
FROM pedidos p
WHERE YEAR(p."fecha de pedido") = 2006
GROUP BY MONTH(p."fecha de pedido");
...
SELECT SUM(p."gastos de envío") AS TotalGastosEnvio
FROM pedidos p;
...
JOIN (SELECT c.id
      ...
      JOIN pedidos p ON c.id = p."id de cliente"
      GROUP BY c.id
      HAVING COUNT(p."id de pedido") > 5) AS clientes_frecuentes
ON p."id de cliente" = clientes_frecuentes.id;
...
FROM pedidos p
JOIN clientes c ON p."id de cliente" = c.id
WHERE p."fecha de pedido" BETWEEN '2006-01-01' AND '2006-01-15';

--20 Obtener el numero rotal de clientes que han realizados pedidos en el año 2006.--
